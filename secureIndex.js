...
http.createServer(server).listen(PORT, hostname, (req, res) => {
  console.log(`Mike is Listening at http://${hostname}:${PORT}/`);
});
https.createServer(creds, server).listen(8443, (req, res) => {
  console.log(
    `Mike secure server is running on https://localhost:${PORT} or 8443/ `
  );
});
